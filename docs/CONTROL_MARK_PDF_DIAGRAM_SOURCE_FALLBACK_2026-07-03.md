# Control mark: PDF diagram source fallback

Date: 2026-07-03
Scope: architecture stabilization / documentation only

This control mark records the current PDF diagram-source selection path before any behavior-preserving cleanup around PDF selected-shape inputs.
...
When no alternative discrete shape is available, the user PDF does not render the technical fallback diagram. It prints a user-facing explanation and a short line with depth, line length, and resolved offset.

This document does not change either path.

## Current fallback/report-text dependency
...
This parsing exists only as a fallback after explicit stores/read-model data are unavailable.

Boundary decision:

```text
PDF should move toward explicit renderer-facing/read-model input.
PDF fallback parsing of reportText should not be expanded.
```
...
This document narrows that finding to the PDF-specific diagram-source selection path and records the exact remaining fallback chain.
