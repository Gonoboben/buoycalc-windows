...
public static class MooringVectorBalance
{
    ...
    public static MooringVectorBalanceResult Build(CalculationResult result)
    {
        ...
        return new MooringVectorBalanceResult(
            ...
            anchorReserve,
            IsSolved: false,
            MethodNote: "Силовые члены собраны в единую векторную ведомость; требуемые реакции вычислены как R = -ΣF. Это проверочная ведомость, а не решение опорных реакций или равновесной формы. При согласованной сумме Fx этот контрольный запас совпадает с запасом удержания якоря в итогах.");
    }
}
