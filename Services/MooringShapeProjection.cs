...
public static class MooringShapeProjection
{
    ...
    public static MooringShapeProjectionResult Build(MooringShapeResult shape)
    {
        ...
        return new MooringShapeProjectionResult(
            ...
            averageAngle,
            geometryClosed,
            "Проекции формы X/Z связывают узлы MooringShapeSolver с геометрией сегментов и используются MooringShapeForceAnalyzer для расчёта силы по нормальной составляющей скорости. Проверка проекций контролирует геометрическое замыкание, но не является полным нелинейным решением равновесия сил и формы.");
    }
}
