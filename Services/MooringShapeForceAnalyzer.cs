...
public static class MooringShapeForceAnalyzer
{
    ...
    public static MooringShapeForceResult Build(CalculationResult result, MooringShapeProjectionResult projection)
    {
        ...
        return new MooringShapeForceResult(
            ...
            maxRowDifference,
            withinTolerance,
            "Для каждого сегмента сила по форме X/Z рассчитывается по нормальной составляющей скорости относительно фактической X/Z-ориентации. Эти силы используются в пересчёте натяжений и итерационном feedback-цикле кандидатной формы, но не заменяют базовый CurrentForceN в CalculationResult и базовых проверках якоря и слабого звена.");
    }
}
