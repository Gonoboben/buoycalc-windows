    ...
    public static MooringShapeForceResult Build(CalculationResult result, MooringShapeProjectionResult projection)
    {
        if (result.SegmentRows.Count == 0 || projection.Rows.Count == 0)
        {
            return new MooringShapeForceResult(
                ...
                0,
                false,
                "Нет сегментов линии или проекций формы для расчёта сил по форме X/Z.");
        }
        ...
        return new MooringShapeForceResult(
            ...
            maxRowDifference,
            withinTolerance,
            "Для каждого сегмента вектор течения shape-based модели в плоскости X/Z строится как (|Uгор|, W): LocalSpeedMS в этой ведомости содержит его модуль, а ShapeForceN рассчитывается по нормальной составляющей относительно фактической X/Z-ориентации. Поэтому W может менять shape-based натяжения и кандидатную/выбранную форму, но эти силы не заменяют базовый CurrentForceN в CalculationResult и базовых проверках якоря и слабого звена.");
    }
}
