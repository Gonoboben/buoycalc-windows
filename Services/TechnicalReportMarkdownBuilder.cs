...
public static class TechnicalReportMarkdownBuilder
{
    ...
    private static void AppendTotals(StringBuilder sb, CalculationResult result, IReadOnlyList<SegmentTensionRow> tensionRows, MooringShapeResult shape, MooringShapeProjectionResult shapeProjection, MooringShapeForceResult shapeForces, MooringShapeTensionResult shapeTensions, MooringSequencePositionResult sequencePositions, MooringDiscreteLoadTensionResult discreteLoadTensions, MooringDiscreteLoadShapeResult discreteLoadShape, MooringAlternativeDiscreteNodeResult alternativeDiscreteNodes, MooringIterativeSolverResult iterativeSolver, EngineeringDiagnosticsResult diagnostics)
    {
        ...
        if (shape.Nodes.Count > 0)
        {
            ...
            sb.AppendLine($"- Сила линии по форме X/Z: {shapeForces.ShapeLineForceN:0.####} Н");
            sb.AppendLine($"- Отличие силы линии по форме X/Z от базовой сегментной модели: {shapeForces.DifferenceN:0.####} Н ({shapeForces.RelativeDifference:0.####})");
            sb.AppendLine($"- Статус сил по форме X/Z: {(shapeForces.WithinTolerance ? "OK" : "INFO: отличается от базовой сегментной модели")}");
            sb.AppendLine($"- Верхнее натяжение базовой сегментной / модели по форме X/Z: {shapeTensions.TopOriginalTensionKn:0.####} / {shapeTensions.TopShapeTensionKn:0.####} кН");
            sb.AppendLine($"- Макс. отличие натяжения модели по форме X/Z: {shapeTensions.MaxTensionDifferenceKn:0.####} кН; угол Δmax={shapeTensions.MaxAngleDifferenceDeg:0.####}°");
            sb.AppendLine($"- Статус натяжений по форме X/Z: {(shapeTensions.WithinTolerance ? "OK" : "INFO: отличается от базовой сегментной модели")}");
            sb.AppendLine($"- Верхнее натяжение базовой сегментной / модели с дискретными нагрузками по s: {discreteLoadTensions.TopOriginalTensionKn:0.####} / {discreteLoadTensions.TopDiscreteTensionKn:0.####} кН");
